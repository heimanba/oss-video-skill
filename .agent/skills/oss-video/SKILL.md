...
```bash
.agent/skills/oss-video/scripts/check-cdp.sh
```
...
```bash
# 从项目根目录执行
.agent/skills/oss-video/scripts/fetch-github-stats.sh {owner}/{repo}

# 示例：获取 alibaba/higress 的信息
.agent/skills/oss-video/scripts/fetch-github-stats.sh alibaba/higress
```
...
```bash
# 从项目根目录执行
.agent/skills/oss-video/scripts/logo-url.sh <homepage_url>

# 示例
.agent/skills/oss-video/scripts/logo-url.sh https://higress.ai
```
